export class EventEmitter<T> {
    ...
    private onPointers: Map<string, Array<usize>> = new Map<string, Array<usize>>()
    private oncePointers: Map<string, Array<i32>> = new Map<string, Array<usize>>()
    constructor() { }
    ...
    on(event: string, callback: (data: T) => void): void {
        if (!this.onPointers.has(event)) {
            this.onPointers.set(event, [callback.index])
        } else {
    ...
    once(event: string, callback: (data: T) => void): void {
        if (!this.oncePointers.has(event)) {
            this.oncePointers.set(event, [callback.index])
        } else {
